...
// Card fronts are generated by scripts/make_project_cards.js.
// techStack: up to 4 entries (one per box on the card back) - a logo path, or a plain name drawn as text.
const FALLBACK_PROJECTS = [
    {
        id: 'fiplive',
        title: 'FIPLIVE',
        front: '/textures/gallery/fipliveprzod.webp',
        painted: '/textures/gallery/fipliveprzod_painted.webp',
        url: 'https://github.com/samfip1/FipLive',
        description: 'Distributed live-streaming platform on Go microservices, Kafka and self-hosted LiveKit. Chat, polls, paid events and VOD for up to 10,000 concurrent viewers, deployed with Terraform + Ansible on Kubernetes.',
        techStack: ['Go', 'Kafka', 'gRPC', 'K8s']
    },
    {
        id: 'forensics',
        title: 'FORENSICS LAB',
        front: '/textures/gallery/forensicsprzod.webp',
        painted: '/textures/gallery/forensicsprzod_painted.webp',
        url: 'https://github.com/samfip1/dynamo-log-report',
        description: 'Linux Forensics Lab-in-a-Box for the NFSU / ISEA faculty program. Every participant gets a ready-made investigation environment, provisioned with OpenTofu and Ansible on Linode.',
        techStack: ['OpenTofu', 'Ansible', 'Linode', 'Nginx']
    },
    {
        id: 'naval',
        title: 'NAVAL COMBAT',
        front: '/textures/gallery/navalprzod.webp',
        painted: '/textures/gallery/navalprzod_painted.webp',
        url: 'https://github.com/samfip1/BattleShip',
        description: 'A browser-based 3D naval combat game with its own small game engine. Physics-based water shaders and custom AABB collision detection, tuned to hold a steady 60 FPS.',
        techStack: ['/textures/gallery/jslogo.webp', '/textures/gallery/htmllogo.webp', '/textures/gallery/csslogo.webp', 'Three.js']
    },
    {
        id: 'kalasetu',
        title: 'KALA SETU',
        front: '/textures/gallery/kalasetuprzod.webp',
        painted: '/textures/gallery/kalasetuprzod_painted.webp',
        url: 'https://kala-setu-phi.vercel.app/',
        description: 'Multi-vendor marketplace where artisans sell products and live learning classes. Redis caching keeps database load low; payments run through Razorpay.',
        techStack: ['/textures/gallery/reactlogo.webp', 'TypeScript', 'Razorpay', 'Redis']
    },
    {
        id: 'wallet',
        title: 'PAY WALLET',
        front: '/textures/gallery/walletprzod.webp',
        painted: '/textures/gallery/walletprzod_painted.webp',
        url: 'https://github.com/samfip1/paytm-Advanced',
        description: 'A secure digital wallet with JWT authentication, peer-to-peer transfers, transaction cashbacks and leaderboards, plus an admin portal to freeze or block flagged accounts.',
        techStack: ['/textures/gallery/reactlogo.webp', 'Node.js', 'MySQL', 'JWT']
    },
    {
        id: 'music',
        title: 'MUSIC LOBBY',
        front: '/textures/gallery/musicprzod.webp',
        painted: '/textures/gallery/musicprzod_painted.webp',
        url: 'https://github.com/samfip1/online-music-player-system',
        description: 'A real-time collaborative music lobby on the Spotify API. Users vote on tracks and a priority-queue algorithm with a 3-minute buffer keeps re-ordering the playback.',
        techStack: ['/textures/gallery/reactlogo.webp', 'Prisma', 'TypeScript', 'Spotify']
    },
];

const PROJECT_COUNT = 12; // Multiple of the project count so the infinite loop repeats cleanly
const GAP = 2.5;
    ...
    const projects = useMemo(() => {
        return Array.from({ length: PROJECT_COUNT }).map((_, i) => {
            ...
            const techStack = projectData.techStack.map(path => {
                if (!canHover || !path.startsWith('/')) return path; // Keep regular / text entries
                ...
                >
                    ...
                    <group position={[0, -0.05, 0.01]}>
                        {project.techStack && project.techStack.map((logoPath, idx) => {
                            // The card back has 4 fixed boxes, so always lay out on the 4-slot grid
                            const spacing = 0.30;
                            const startX = -(3 * spacing) / 2;
...
const TechStackLogo = ({ path, position }) => {
    if (!path.startsWith('/')) return <TechStackName name={path} position={position} />;
    return <TechStackImage path={path} position={position} />;
};

const TechStackName = ({ name, position }) => (
    <Text
        position={position}
        fontSize={Math.min(0.065, 0.4 / name.length)}
        color="#1c1c1c"
        font="/fonts/CabinSketch-Bold.ttf"
        anchorX="center"
        anchorY="middle"
...
const TechStackImage = ({ path, position }) => {
